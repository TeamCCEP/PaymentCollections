...
class PayWidget extends HTMLElement {
  ...
  connectedCallback() {
    ...
    let triggerPaymentFetchURL =
      "https://ignore.me.hooks.imiconnect.eu/events/"; // Not in use any longer;
    let checkPaymentFetchURL =
      "https://hooks.us.webexconnect.io/events/"; //Webhook address for flow to be called when payment is confirmed;
    let checkPaymentAuthKey = "c3683c03-9a40-11dddec-9d17-0aaac5b19840"; // Carles "656a7e4d-899b-11ec-8286-0645430715b9"
    let FlyHighBaseURL = "https://ignore.meflyhighbookings.duckdns.org";
    ...
    try {
      if (this.currentTask) {
        if(this.currentTask.mediaChannel=="broadcloud") { // Voice call
          ...
          if(this.currentTask.contactDirection == "INBOUND") { //INBOUND
            ...
            if (this.currentTask.callAssociatedData.Charge) { //OUTBOUND
                ...
                customerName = this.currentTask.callAssociatedData.Customer_Name.value; //CustomerName expected as Call Variable}  
                setInboundDisplay();
            } else {
   ...
   function getcustomername(number) {
      ...
      console.log(
        "[PAYWIDGET] - Sending request to " + FlyHighBaseURL + "/cfa/getuserdata?ani=" + number + " with request options:"
      );
      console.log(requestOptions);
    ...
    function getcustomernamefromPSID(PSID) {
      ...
      console.log(
        "[PAYWIDGET] - Sending request to " + FlyHighBaseURL + "/cfa/getuserdata?psid=" + PSID + " with request options:"
      );
      console.log(requestOptions);
    ...
    function getcustomernamefromEmail(Email) {
      ...
      console.log(
        "[PAYWIDGET] - Sending request to " + FlyHighBaseURL + "/cfa/getuserdata?email=" + Email + " with request options:"
      );
      console.log(requestOptions);
